...
function transitionToLoading() {
  ...
  setTimeout(() => {
    ...
    // Ensure a progress bar exists for polling feedback
...
function handleSubmissionSuccess(response) {
  ...
  if (response && (response.analysis_complete || response.hail_probability !== undefined)) {
    ...
    // Add a manual check button if not exists
    if (!document.getElementById('manualCheckBtn')) {
      ...
      btn.onclick = async () => {
        // Attempt one immediate poll
        elements.statusMessage.textContent = 'Checking for results...';
        try {
          const res = await pollForResults(state.currentRequestId || generateRequestId(), collectFormData());
          if (res && (res.analysis_complete || res.hail_probability !== undefined)) {
      ...
      if (actionContainer) actionContainer.appendChild(btn);
    }
...
function updateProgressPercentage(percentage) {
  ...
  const fill = bar.querySelector('.poll-fill');
  const text = bar.querySelector('.poll-text');
  if (fill) fill.style.width = `${percentage}%`;
  if (text) text.textContent = `${percentage}%`;
  ...
  // When complete, fade out after a short delay
  if (percentage >= 100) {
    console.log('Progress complete - cleaning up progress bar');
    setTimeout(() => {
      const existing = document.querySelector('.poll-progress');
